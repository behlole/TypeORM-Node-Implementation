"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const User_1 = __importDefault(require("../models/User"));
const Books_1 = __importDefault(require("../models/Books"));
const RequestResponseMappings_1 = __importDefault(require("../../../Shared/utils/Mappings/RequestResponseMappings"));
exports.default = {
    addBook: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        let bookNames = req.body.bookNames;
        let user = req.body.user;
        let currentUser = yield User_1.default.findOneBy({ email: user.email });
        if (currentUser) {
            let books = [];
            for (const singleBook of bookNames) {
                let book = Books_1.default.create({ bookName: singleBook });
                yield book.save();
                books.push(book);
            }
            currentUser.books = books;
            yield currentUser.save();
            return RequestResponseMappings_1.default.sendSuccessMessage(res, currentUser);
        }
        return RequestResponseMappings_1.default.sendErrorMessage(res);
    }),
    getBooks: (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            if (req.params && req.params.id) {
                let book = yield Books_1.default.findOne({
                    where: {
                        id: parseInt(req.params.id)
                    },
                    relations: {
                        user: true
                    }
                });
                return RequestResponseMappings_1.default.sendSuccessMessage(res, book);
            }
            else {
                let books = yield Books_1.default.find({
                    relations: {
                        user: true
                    }
                });
                return RequestResponseMappings_1.default.sendSuccessMessage(res, books);
            }
        }
        catch (e) {
            return RequestResponseMappings_1.default.sendErrorMessage(res, e.message);
        }
    }),
};
